...
Original file is located at
    https://colab.research.google.com/drive/1GbwkhivFH4CDQ4bu3y24vtGEtg5PAnSW
"""
...
high_paths = collect_paths_v2('High_Resolution')[:1500]
low_paths = collect_paths_v2('Low_Resolution')[:1500]
...
def perceptual_loss(preds, targets, vgg_model):
    preds_vgg = vgg_model(preds)
    targets_vgg = vgg_model(targets)
    return torch.nn.functional.l1_loss(preds_vgg, targets_vgg)

class VGGPerceptual(nn.Module):
    def __init__(self):
        super().__init__()
        vgg = torchvision.models.vgg16(weights=torchvision.models.VGG16_Weights.IMAGENET1K_FEATURES).features[:16]
        for param in vgg.parameters():
            param.requires_grad = False
        self.vgg = vgg

    def forward(self, x):
        # VGG expects input in [0,1] normalized to ImageNet stats
        x = torchvision.transforms.functional.resize(x, [224, 224])
        x = (x - 0.5) / 0.5  # Normalize to roughly [-1,1]
        return self.vgg(x)
...
def motion_weighted_loss(preds, targets, flows, base_loss):
    # flows: (B, 2, H, W), flow x/y normalized between [-1,1]
    motion_mag = torch.norm(flows, dim=1, keepdim=True)  # (B,1,H,W)
    weights = (motion_mag > 0.05).float() * 0.5 + 1.0  # 給有動作的地方較高權重
...
#train_enhancer_only(model, enhancer, dataloader, optimizer, epochs=10)

torch.save(enhancer.state_dict(), "/content/drive/MyDrive/enchaner.pth")
...
def apply_edge_enhancement_lab(tensor_img, amount=1.5, radius=1, saturation_scale=1.2):
    """
